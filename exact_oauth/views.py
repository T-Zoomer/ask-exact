...
def authorize(request):
    ...
    if not config['client_id'] or not config['client_secret']:
        messages.error(request, 'Exact Online not configured. Set EXACT_CLIENT_ID and EXACT_CLIENT_SECRET environment variables.')
...
def callback(request):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            token, created = ExactOnlineToken.objects.get_or_create(session_key=auth_state.session_key)
...
def test_api(request):
    ...
    try:
        ...
        # Test call to get current user info
